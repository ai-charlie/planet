...
# fmt: off
assert (_determine_versions("2021.08.17.dev44",
                            date=datetime.date(
                                2021, 8,
                                17)) == ("2021.08.17.44",
                                         "2021.08.17.dev45")), "same day 1"
assert (_determine_versions("2021.08.17.dev1", date=datetime.date(
    2021, 8, 17)) == ("2021.08.17.1", "2021.08.17.dev2")), "same day 2"
assert (_determine_versions("2021.08.17.dev44",
                            date=datetime.date(
                                2021, 8,
                                18)) == ("2021.08.18",
                                         "2021.08.18.dev1")), "different day"
# fmt: on
...
def release(session):
    ...
    allowed_upstreams = [
        f"git@github.com:ai-charlie/{PACKAGE_NAME.replace('_', '-')}.git"
    ]
    ...
    session.run("release-helper", "git-check-tag", release_version,
                "--does-not-exist")
    session.run("release-helper", "git-check-remote", "origin",
                *allowed_upstreams)

    # Prepare release commit
    session.run("release-helper", "version-bump", version_file,
                release_version)
    ...
    session.run("git",
                "commit",
                "-m",
                f"Prepare release: {release_version}",
                external=True)
